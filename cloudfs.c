...
/*Cache Structure used to reduce cloud cost*/
...
void *cloudfs_init(struct fuse_conn_info *conn UNUSED)
{
  ...
  fprintf(logfile, "The avg, min and max and window are %d %d %d %d\n", avg_seg_size, min_seg_size, max_seg_size, window_size);
  fprintf(logfile, "The segmentation is %d\n", segmentation);
  fprintf(logfile, "The cache size is %lu\n", total_cache_size);
  fflush(logfile);
  ...
  fprintf(logfile, "Initialization is done\n");
  fflush(logfile);
  return NULL;
}
...
int cache_dir_list_initialization()
{
  ...
  if (dp == NULL)
  {
    fprintf(logfile,"Cache does not exist\n");
    fflush(logfile);
  ...
  {
    fprintf(logfile,"Cache already exists\n");
    fflush(logfile);
 ...
 * This function does the following
 * 1) It builds the in-memory linkedlist from the persistant metacache file
...
void cloudfs_segmentation(char *fpath, int argument)
{
  ...
  /*Populates the per-file memory structure from linked list*/
  cloudfs_populate_metafile(size, fpath, &metafile_length,&head, &tail);
  /* Once you have populated the per-file memory structure which is persistent
